...
BROTLI_COMMON_API void BrotliSharedDictionaryCleanup(
    BrotliSharedDictionary* dict) {
  dict->free_func(dict->memory_manager_opaque, dict->words_instances);
  dict->free_func(dict->memory_manager_opaque, dict->transforms_instances);
  dict->free_func(dict->memory_manager_opaque, dict->prefix_suffix_maps);
}

void BrotliSharedDictionaryInitDefault(BrotliSharedDictionary* dict,
...
BROTLI_BOOL BrotliSharedDictionaryInit(
    BrotliSharedDictionary* dict, BrotliSharedDictionaryType type,
    const uint8_t* data, size_t size,
    brotli_alloc_func alloc_func, brotli_free_func free_func, void* opaque) {
  BrotliSharedDictionaryInitDefault(dict, alloc_func, free_func, opaque);
  return BrotliSharedDictionaryAttach(dict, type, data, size);
}
